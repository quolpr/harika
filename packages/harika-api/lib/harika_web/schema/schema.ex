defmodule HarikaWeb.Schema do
  ...
  object :session do
    ...
    field :db_auth_token, non_null(:string) do
      resolve(fn params, _parent, %{context: context} ->
        {:ok, params[:db_auth_token] || Accounts.get_couchdb_token(context[:current_user])}
      end)
    end
  end
  ...
  query do
    field :current_session, :session do
  ...
  mutation do
    ...
    field :create_vault_db, non_null(:boolean) do
      arg(:db_id, non_null(:string))

      resolve(&AccountsResolver.create_vault_db/3)
    end
  end
end
